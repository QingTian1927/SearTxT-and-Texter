...
def write_settings(settings_dir, arguments):
    if os.path.exists(settings_dir):
        old_file = f"{settings_dir.rstrip('.conf')}_old.conf"
...
# Single-threaded
def exact_search(query, search_dir):
    found = 0
            ...
            with open(file_dir, 'r', encoding='utf8') as searched_file:
                for index, line in enumerate(searched_file, start=1):
                    if query.lower() in line.lower():
                        print(f"{Colors.GREEN}##{Colors.RESET} 1 match at {Colors.BLUE}Line({index}) {Colors.RESET}"
                              f" of {Colors.BLUE}{file_name}{Colors.RESET}")
        ...
        while softwaring:
            ...
            while searching:
                ...
                if search_method == 'exact_match':
                    result_counter += exact_search(user_input, target_dir)
