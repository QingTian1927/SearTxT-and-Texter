...
COMMANDS = [
    ...
    '/q                  : exit the program',
    '/t (thread)         : allocate a number of cpu threads to the approximate searcher\n'
]
...
def write_settings(file_dir, arguments):
    if os.path.exists(file_dir):
        old_file = f"{file_dir.rstrip('.conf')}_old.conf"
        os.replace(file_dir, old_file)

    with open(file_dir, 'w', encoding='utf8') as file:
        for line in arguments:
            file.write(f"{line}\n")


def read_settings(file_dir):
    settings_target_dir = ''
...
def approximate_search(args):
    ...
    # TODO: Make get_close_matches(cutoff) tunable & print multiple similar results
    if file_name.endswith('.txt'):
        ...
        with open(file_dir, 'r', encoding='utf8') as searched_file:
            for index, line in enumerate(searched_file, start=1):
                match = get_close_matches(query.lower(), line.lower().split(), 1, 0.8)
                if match:
        ...
        while softwaring:
            ...
            if user_input == '/c':
                ...
                if allocation_changed:
                    print(f"Allocated ({allocated_threads}) cpu threads to the approximate searcher")
            elif user_input == '/h':
            ...
            while searching:
                ...
                for file_entry in os.listdir(target_dir):
                    search_args.append(f"{file_entry}#@$@#{user_input}#@$@#{target_dir}")
